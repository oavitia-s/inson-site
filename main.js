...
(function initNavbar() {
  ...
  style.textContent =
    ".nav-links a.active { color: #fff !important; } .nav-links a.active::after { right: 0 !important; }";
...
(function initContactForm() {
  ...
  function validate() {
    for (const [key, { el, label }] of Object.entries(fields)) {
      ...
      if (!val) {
        showNote(`${label} is required.`, "error");
        el.focus();
        return false;
      }
...
function openSmartRedirect() {
  ...
  if (isMobile) {
    ...
    window.open(
      `https://web.whatsapp.com/send?phone=${PHONE}&text=${encodeURIComponent("Hola, me gustaría recibir información sobre su escuela, por favor.")}`,
      "_blank",
    );
  }
}

const whatsappBtn = document.querySelector(".fixed-wa-btn");
const triggerSection = document.querySelector("#description"); // tu sección 2

window.addEventListener("scroll", () => {
  if (triggerSection && whatsappBtn) {
